...
def load_bdd10k_panoptic_json(json_file, image_dir, gt_dir, meta):
    def _convert_category_id(segment_info, meta):
        if str(segment_info["category_id"]) in meta["thing_dataset_id_to_contiguous_id"]:
...
def register_bdd10k_panoptic(
    ...
    metadata = get_metadata(BDD10K_CATEGORIES)
...
_PREDEFINED_SPLITS_BDD10K_PANOPTIC = {
    "bdd10k_panoptic_train": (
        "bdd10k/images/train",
        "bdd10k/labels/jsons/pan_seg/masks/train",
...
def get_metadata(BDD10K_CATEGORIES):
    meta = {}
    thing_classes = [k["name"] for k in BDD10K_CATEGORIES if k["isthing"] == True]
    thing_colors = [k["color"] for k in BDD10K_CATEGORIES if k["isthing"] == True]
    ...
    for i, cat in enumerate(BDD10K_CATEGORIES):
        if cat["isthing"]:
            thing_dataset_id_to_contiguous_id[str(cat["id"])] = i
        else:
